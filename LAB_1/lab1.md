...
- [Отчет по лабораторной работе №1](#отчет-по-лабораторной-работе-1)
  - [Высокодоступный кластер PostgreSQL (Patroni + Zookeeper + HAProxy)](#высокодоступный-кластер-postgresql-patroni--zookeeper--haproxy)
  ...
  - [4 Выполнение основного задания (Failover Test)](#4-выполнение-основного-задания-failover-test)
    - [Шаг 1. Принудительная остановка мастера](#шаг-1-принудительная-остановка-мастера)
    - [Шаг 2. Автоматическое переключение (Failover)](#шаг-2-автоматическое-переключение-failover)
    - [Шаг 3. Проверка работоспособности через HAProxy](#шаг-3-проверка-работоспособности-через-haproxy)
    - [Объяснение произошедшего](#объяснение-произошедшего)
    - [Шаг 4. Восстановление кластера (Auto-Recovery)](#шаг-4-восстановление-кластера-auto-recovery)
  - [5 Ответы на вопросы](#5-ответы-на-вопросы)
    - [5.1 В чем разница между `expose` и `ports`?](#51-в-чем-разница-между-expose-и-ports)
    - [5.2 Собирается ли образ заново при перезапуске проекта?](#52-собирается-ли-образ-заново-при-перезапуске-проекта)
...
INSERT INTO lab_test (message)
VALUES ('Data from Master');

INSERT INTO lab_test (message)
VALUES ('Second data from Master');
```
...
После перезапуска проекта в Части 3 роли были распределены заново: лидером стал контейнер `pg-slave` (`postgresql1`), а `pg-master` (`postgresql0`) перешел в роль реплики. Лидер выбирается через Zookeeper и не привязан к имени контейнера. Текущий лидер `pg-slave` был остановлен командой:
...
Для проверки возможности записи через единую точку входа было выполнено подключение к HAProxy на порту 5432:

```bash
psql -h localhost -p 5432 -U postgres -d postgres
```

После переключения была добавлена новая запись:

```sql
INSERT INTO lab_test (message)
VALUES ('Data after failover - New Master');
...
После остановки лидера Patroni на этом узле перестал обновлять ключ лидера в Zookeeper. По истечении ttl (30 с) блокировка была потеряна, и оставшийся узел, обнаружив отсутствие лидера, повысил себя до роли лидера.

HAProxy проверяет состояние узлов через REST API Patroni (порт 8008): код 200 возвращает только лидер. Остановленный узел не отвечал, поэтому после трех неудачных проверок (`fall 3`, интервал 3 с) был помечен как DOWN, а новый лидер получил статус UP. Существующие сессии были закрыты (`shutdown-sessions`), поэтому клиенту потребовалось переподключиться. Время недоступности определяется по меткам времени в логах.
...
### 5.1 В чем разница между `expose` и `ports`?

`expose` открывает порт только внутри Docker-сети. Это значит, что к нему могут обращаться другие контейнеры, но напрямую с компьютера подключиться нельзя.

`ports` используется, когда порт контейнера нужно открыть на хост-машине. Например, запись `5433:5432` означает, что порт 5432 внутри контейнера доступен на компьютере через порт 5433.

В нашем проекте порт 8008 нужен для взаимодействия HAProxy с REST API Patroni внутри Docker-сети, поэтому для него используется `expose`. Порты PostgreSQL опубликованы через `ports`, чтобы мы могли подключаться к базе данных с компьютера.

### 5.2 Собирается ли образ заново при перезапуске проекта?

При обычном перезапуске проекта Docker не собирает образ заново, а запускает уже существующий образ.

Файлы `postgres0.yml` и `postgres1.yml` копируются внутрь образа во время сборки. Поэтому после изменения этих файлов или самого `Dockerfile` образ нужно пересобрать:
...
- автоматический failover при отказе мастера: после кратковременной недоступности (порядка ttl = 30 с) HAProxy автоматически перенаправил клиентов на нового лидера;
